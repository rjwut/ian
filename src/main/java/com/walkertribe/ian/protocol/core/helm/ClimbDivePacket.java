 ...
 * @author rjwut
 */
public class ClimbDivePacket extends ShipActionPacket {
    private static final int UP = -1;
    private static final int DOWN = 1;

	public static void register(PacketFactoryRegistry registry) {
		registry.register(ConnectionType.CLIENT, TYPE, TYPE_CLIMB_DIVE,
				new PacketFactory() {
			@Override
    ...
    public ClimbDivePacket(boolean up) {
        super(TYPE_CLIMB_DIVE, up ? UP : DOWN);
    }

    private ClimbDivePacket(PacketReader reader) {
        super(TYPE_CLIMB_DIVE, reader);
    }
